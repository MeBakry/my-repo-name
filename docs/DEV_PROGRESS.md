...
| File | What's in it |
|------|-------------|
| `docs/DEV_PROGRESS.md` | **This file** — current state, all decisions, what to do next |
| `docs/HOW_IT_WORKS.md` | Plain-language explanation of the product |
| `docs/SETUP_GUIDE.md` | How to set up the local environment (macOS Apple Silicon) |
| `docs/TESTING.md` | Full test plan: curl commands + UI test cases |
| `docs/index.html` | Technical documentation hub (open in browser) |
| `poc-pharma-risk/TEST_CASES.md` | UI test cases |
| `server/README.md` | Backend API reference, demo accounts |
| `server/prisma/schema.prisma` | Database schema |
| `server/.env` | Database connection string (not in git) |
...
8 Models, 11 Enums (see `server/prisma/schema.prisma` for full detail)
...
cd server
cp .env.example .env
# Edit .env: DATABASE_URL="postgresql://YOUR_MACOS_USERNAME@localhost:5432/pharma_compliance?schema=public"
# (No password for Homebrew PostgreSQL)
npm install
