 ...
 *
 * In production (Docker), VITE_API_URL is set to "/api" so requests
 * go to the same origin and Nginx proxies to the backend.
 */
...
export async function rejectAssessment(id, notes) {
...
export async function deactivateAssessment(id, notes) {
  return apiFetch(`/assessments/${id}/deactivate`, {
    method: "PUT",
    body: JSON.stringify({ notes }),
  });
}
