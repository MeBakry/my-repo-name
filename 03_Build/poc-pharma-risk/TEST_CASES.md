...
> **Note:** The full testing guide including backend API tests is in `03_Build/TESTING.md`.
...
1. Start the backend server (Terminal 1):
   ```bash
   cd 03_Build/server && npm run dev
   # Or from repo root: npm run dev:backend
   # Should show: Server running on port 3001
   ```
2. Start the frontend (Terminal 2):
   ```bash
   cd 03_Build/poc-pharma-risk && npm run dev
   # Or from repo root: npm run dev:frontend
   ```
3. Open `http://localhost:3000` (or http://localhost:5173) in your browser
